...
Private Sub Form_Load()
    ...
    'Set the dimensions of the image list
...
Private Sub Form_Resize()
    ...
    'This is an easy way to fit a control to the size of a form
    lstFiles.Move 0, 2400, ScaleWidth, ScaleHeight - stbDragDrop.Height - 2400
    ...
    'Set the width of the status bar panel
...
errHandler:
    ...
    MsgBox Err.Description, vbCritical, "Crypto"
    picSmall.Picture = LoadPicture()
...
Private Sub lstFiles_OLEDragOver(Data As MSComctlLib.DataObject, Effect As Long, Button As Integer, Shift As Integer, x As Single, y As Single, State As Integer)
    ...
    'Set effect to 1 so the little plus sign shows up by the mouse pointer
    Effect = 1
...
Public Sub ListView_OLEDragDrop(Data As MSComctlLib.DataObject, ListView As MSComctlLib.ListView)
    ...
    For i = 1 To FileCount
        ...
        Else
            ...
            'Call the procedure to get the total file size
            ...
            If ListView.ListItems.Count > 255 Then
                MsgBox "Only 255 Files can be inserted.", vbInformation, "Crypto"
                Exit Sub
            End If
            ...
            If Not FileExists(FileName) Then
                'Icon is not there, add it to the image list
...
errHandler:
    MsgBox "An error was encountered while trying to add " & Data.Files(i) & "." & _
        " Make sure the no other users or applications are accessing the file.", vbExclamation, "Crypto"
...
Public Function CreateArchive(Path As String) As Boolean
    ...
    'Use | as delimiter, asc = 124
    TotalLength = GetTotalFileSize
    FileCount = lstFiles.ListItems.Count
    
    If TotalLength = -1 Then
        MsgBox "The Archive is too large. Some files need to be removed.", vbExclamation, "Crypto"
        CreateArchive = False
        Exit Function
    End If
    ...
    'Add space for trailer
    ReDim Preserve b(UBound(b) + 2)
    
    'Add the trailer - used for decoding
    b(UBound(b) - 2) = Asc("C")
    b(UBound(b) - 1) = Asc("P")
    b(UBound(b)) = Asc("T")
    ...
    MsgBox "Files Archived at " & Path, vbInformation, "Crypto"
...
Private Sub tlbCrypto_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    ElseIf Button.Index = 3 Then
        ...
        'Checking for valid input is really tedious, so this part is kind of long.
        
        If lstKey.ListItems.Count <> 1 Then
            MsgBox "Missing Key. Please drag a file to use as the key into the box.", vbExclamation, "Crypto"
            Exit Sub
        End If
        If txtPath.Text = "" Then
            MsgBox "The Path specified as the File Destination does not exist, please enter a correct Path.", vbExclamation, "Crypto"
            txtPath.SetFocus
            Exit Sub
        End If
        ...
        If optAction(0) Then
            If lstFiles.ListItems.Count = 0 Then
                MsgBox "No files were selected to Encrypt. Please make sure there is at least one file in the list.", vbExclamation, "Crypto"
                Exit Sub
            End If
        ElseIf optAction(1) Then
            If lstFiles.ListItems.Count <> 1 Then
                MsgBox "Only one *.cpt file can be Decrypted at once. Please make sure there is only one valid Archive in the list.", vbExclamation, "Crypto"
                Exit Sub
            End If
            If lstFiles.ListItems.Count = 1 Then
                If UCase(Right(lstFiles.ListItems(1).Text, 3)) <> "CPT" Then
                    MsgBox "The file you are attempting to Decrypt is not a valid *.cpt file. Please make sure there is only one valid Archive in the list.", vbExclamation, "Crypto"
                    Exit Sub
                End If
            End If
        End If
        ...
        '***************************************************
        'Everything is ok, procede with selected operation
        '***************************************************
        ...
        If optAction(0) Then
            ...
            If Not CreateArchive(Path) Then
                MsgBox "The files were unable to be Encrypted.", vbExclamation, "Crypto"
            End If
        ...
        ElseIf optAction(1) Then
            ...
            If Not DecodeArchive(SourcePath, Path) Then
                MsgBox "The Archive was not able to be Decrypted.", vbExclamation, "Crypto"
            End If
